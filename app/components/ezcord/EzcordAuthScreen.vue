    ...
    <div class="w-full max-w-[520px] rounded-[18px] border border-ez-line bg-gradient-to-b from-ez-card to-ez-card-2 p-[22px] shadow-ez">
      ...
      <form class="mt-[18px] grid gap-3.5" @submit.prevent="emit('submit')">
        ...
        <button
          ...
          type="submit"
        >
          <EzcordSpinner v-if="props.isLoading" class="mr-2 h-4 w-4" />
      ...
      >
        <EzcordSpinner v-if="props.telegramLoginLoading" class="h-5 w-5 shrink-0" />
        <svg v-else class="h-5 w-5 shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" aria-hidden="true">
